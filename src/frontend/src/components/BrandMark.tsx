...
function BrandMark({ compact = false }: BrandMarkProps) {
  ...
  return (
    <span
      className={`group relative inline-flex [perspective:900px] ${compact ? 'h-10 w-10' : 'h-56 w-56 md:h-72 md:w-72'}`}
    ...
    >
      ...
      <svg viewBox="0 0 300 300" className="relative z-10 h-full w-full overflow-visible transition-transform duration-100" style={{ transform: `rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(${scale})` }} fill="none">
        ...
        <g className="transition duration-500 group-hover:-translate-y-1">
